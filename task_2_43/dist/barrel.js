...
(function (window) {
  ...
  ZdAlbum.prototype.setImages = function (images, options) {
    ...
    while (images.length) {
      ...
      var result = generateRow.call(this, images);
      var imgs = images.splice(0, result.rowNum);
    ...
    function generateRow(imgs) {
      ...
      var rh = this.targetRowHeight;
      var minh = rh * (1 - this.targetRowHeightTolerance);
      var maxh = rh * (1 + this.targetRowHeightTolerance);
      var rowNum = -1;
      for (var i = 0; i < imgs.length; i++) {

        var nw = rw + imgs[i].ratio;
        var nh = w / nw;
        if (nh < minh) {
          rowNum = i;
          break;
